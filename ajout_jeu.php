    ...
    <main class="container p-5">
      ...
      <div>
        <form action="verification_jeu.php" method="post" enctype="multipart/form-data">
          ...
          echo '<div class="row">
          <label for="genre" class="form-label"><h4>Genre du jeu</h4></label>';

          for ($i=0; $i < $count; $i++) {
            if ($i%6 == 0 AND $i != 0) {
              echo
              '<input type="checkbox" class="btn-check" id="'.$fetch[$i].'" name="genre[]" value="'.$fetch[$i].'" autocomplete="off">
              <label class="btn btn-outline-primary col mx-2" for="'.$fetch[$i].'">'.$fetch[$i].'</label>
              <div>
              ';
            } else {
              echo '<input type="checkbox" class="btn-check" id="'.$fetch[$i].'" name="genre[]" value="'.$fetch[$i].'" autocomplete="off">
              <label class="btn btn-outline-primary col mx-2" for="'.$fetch[$i].'">'.$fetch[$i].'</label>';
            }
          }
          if (($count+1)%6 != 0) {
